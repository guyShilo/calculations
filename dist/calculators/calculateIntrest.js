...
function getIntrestByPercentages(result) {
    var _this = this;
    var monthlyPayment = result.monthlyPayment, percentage = result.percentage, period = result.months, initialAmount = result.initialAmount;
    function countYears(count) {
        if (count == 12) {
            counter = 0;
            year++;
            // @ts-ignore
            var emptyRecord = UTILS.generateRecord('-> ->', '-> ->', '-> ->', 'YEAR', '-> ->', '-> ->', '-> ->', '-> ->', 'PASSED', '-> ->');
            monthlyRecord.push(emptyRecord);
        }
        return counter++;
    }
    var months = parseInt(lodash_1.default.toString(period * 12));
...
exports.default = getIntrestByPercentages;
